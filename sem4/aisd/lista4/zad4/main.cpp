...
#include "../zad2/include/Aggregator.hpp"
#include "../zad3/include/SplayTree.hpp"
...
int main() {
  ...
  for(int n = START; n <= STOP; n += STEP) {
    ...
    for(int test = 0; test < TESTS; test++) {
      ...
      std::vector<int> randSequenceInsert = ascSequence;
      std::shuffle(randSequenceInsert.begin(), randSequenceInsert.end(), localRNG);

      std::vector<int> randSequenceDeleteA = ascSequence;
      std::shuffle(randSequenceDeleteA.begin(), randSequenceDeleteA.end(), localRNG);

      std::vector<int> randSequenceDeleteB = ascSequence;
      std::shuffle(randSequenceDeleteB.begin(), randSequenceDeleteB.end(), localRNG);
      ...
      for(int i = 0; i < n; i++) {
        s.reset();
        treeA.deleteKey(randSequenceDeleteA[i], s);
      ...
      for(int i = 0; i < n; i++) {
        s.reset();
        treeB.insertNode(randSequenceInsert[i], s);
        localInsertAggB.update(s.cmp, s.ptr, treeB.height());
      }

      for(int i = 0; i < n; i++) {
        s.reset();
        treeB.deleteKey(randSequenceDeleteB[i], s);
